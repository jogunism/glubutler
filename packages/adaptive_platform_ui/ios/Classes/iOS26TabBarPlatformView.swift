...
class iOS26TabBarPlatformView: NSObject, FlutterPlatformView, UITabBarDelegate {
    ...
    init(frame: CGRect, viewId: Int64, args: Any?, messenger: FlutterBinaryMessenger) {
        ...
        if let dict = args as? [String: Any] {
            ...
            _ = (dict["spacerFlags"] as? [Bool]) ?? [] // Read but unused
                ...
                if isSearch {
                    ...
                    if i < symbols.count && !symbols[i].isEmpty {
                        ...
                        if #available(iOS 26.0, *) {
                            ...
                            // Selected: Use selectedSymbols if available, otherwise fall back to symbols
                            let selectedSymbolName = (i < selectedSymbols.count && !selectedSymbols[i].isEmpty) ? selectedSymbols[i] : symbols[i]
                            selectedImage = UIImage(systemName: selectedSymbolName)?.withRenderingMode(.alwaysTemplate)
                        } else {
            ...
            if isSearch {
                ...
                if i < currentSymbols.count && !currentSymbols[i].isEmpty {
                    if #available(iOS 26.0, *) {
                        ...
                        // Selected: Use currentSelectedSymbols if available, otherwise fall back to currentSymbols
                        let selectedSymbolName = (i < currentSelectedSymbols.count && !currentSelectedSymbols[i].isEmpty) ? currentSelectedSymbols[i] : currentSymbols[i]
                        selectedImage = UIImage(systemName: selectedSymbolName)?.withRenderingMode(.alwaysTemplate)
                    } else {
